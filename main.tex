...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[node distance=4cm, auto,
        ...
        \node[block, right of=eng] (manual) {Formulação Manual (Matemática/Código)};
        \node[block, right of=manual] (solver) {Solver de Otimização};
        
        % Labels
        \node[above=0.5cm of manual, font=\bfseries] {Abordagem Tradicional};
        
        \path[line] (eng) -- node[midway, above] {Conhecimento} (manual);
        \path[line] (manual) -- (solver);
        \path[line, dashed] (solver) -- ++(0,-1.5) -| node[pos=0.25, below] {Erro/Feedback Manual} (eng);

        % Cenário Proposto (human-in-the-loop)
        \node[human, below=3.5cm of eng] (eng2) {Engenheiro Especialista};
        \node[block, right of=eng2, fill=orange!10] (mas) {\textbf{Agentes Colaborativos} (Formulador + Crítico)};
        \node[block, right of=mas] (solver2) {Solver de Otimização};

        % Labels
        \node[above=0.5cm of mas, font=\bfseries] {Abordagem Proposta};

        \path[line] (eng2) -- node[midway, above] {Linguagem Natural} (mas);
        \path[line] (mas) -- node[midway, above] {Modelo} (solver2);
        \path[line] (solver2) -- node[midway, below] {Viabilidade} (mas);
        \path[line, dashed] (mas) -- node[midway, above] {Refinamento} (eng2);
