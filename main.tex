...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \node[block, right=2cm of eng] (manual) {Formulação Manual (Matemática/Código)};
        \node[block, right=1.5cm of manual] (solver) {Solver de Otimização};
        ...
        \node[human, below=4cm of eng] (eng2) {Engenheiro Especialista};
        \node[block, right=2cm of eng2, fill=orange!10] (mas) {\textbf{Agentes Colaborativos} (Formulador + Crítico)};
        \node[block, right=1.5cm of mas] (solver2) {Solver de Otimização};

        % Labels
        \node[above=0.2cm of mas, font=\bfseries] {Abordagem Proposta};

        % Ajustando setas e labels para não sobrepor
        \path[line] (eng2) -- node[midway, above, align=center] {Linguagem\\Natural} (mas);
        \path[line] (mas) -- node[midway, above] {Modelo} (solver2);
        \path[line] (solver2) -- node[midway, below] {Viabilidade} (mas);
        \path[line, dashed] (mas) -- node[midway, above, Sloped] {Refinamento} (eng2);
