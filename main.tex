...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[node distance=1.5cm, auto,
        ...
        block/.style={rectangle, draw, rounded corners, fill=blue!10, text width=2.5cm, align=center, minimum height=1.2cm},
        human/.style={ellipse, draw, fill=green!10, text width=2cm, align=center},
        cloud/.style={cloud, draw, fill=gray!10, aspect=2},
        line/.style={draw, -latex', thick}]

        % Cenário Tradicional
        \node[human] (eng) {Engenheiro Especialista};
        \node[block, right=1cm of eng] (manual) {Formulação Manual (Matemática/Código)};
        \node[block, right=1cm of manual] (solver) {Solver de Otimização};
        
        % Labels
        \node[above=0.2cm of manual, font=\bfseries] {Abordagem Tradicional};
        
        \path[line] (eng) -- node[above] {Conhecimento} (manual);
        \path[line] (manual) -- (solver);
        \path[line, dashed] (solver) -- ++(0,-1) -| node[pos=0.25, below] {Erro/Feedback Manual} (eng);

        % Cenário Proposto (human-in-the-loop)
        \node[human, below=2.5cm of eng] (eng2) {Engenheiro Especialista};
        \node[block, right=1cm of eng2, fill=orange!10] (mas) {\textbf{Agentes Colaborativos} (Formulador + Crítico)};
        \node[block, right=1cm of mas] (solver2) {Solver de Otimização};

        % Labels
        \node[above=0.2cm of mas, font=\bfseries] {Abordagem Proposta};

        \path[line] (eng2) -- node[above] {Linguagem Natural} (mas);
        \path[line] (mas) -- node[above] {Modelo} (solver2);
        \path[line] (solver2) -- node[above] {Viabilidade} (mas);
        \path[line, dashed] (mas) -- node[above] {Refinamento} (eng2);

    \end{tikzpicture}
    \caption{Comparativo conceitual: (Superior) O fluxo tradicional exige que o engenheiro traduza manualmente requisitos em formalismo matemático. (Inferior) A abordagem proposta intercala agentes inteligentes que traduzem linguagem natural e validam tecnicamente o modelo antes da execução.}
